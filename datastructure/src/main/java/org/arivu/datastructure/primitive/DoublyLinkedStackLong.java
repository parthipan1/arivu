...
//	static final AtomicWFLock cas = new AtomicWFLock();
	 ...
	 * 
	 */
	DoublyLinkedStackLong left = this, right = this;
	...
	public int search(final long o){
		int idx = 0;
	 ...
	 * @return
	 */
	DoublyLinkedStackLong removeRight(){
		final DoublyLinkedStackLong r = this.right;
		if( r == this || r == null ){
			return null;
		}else{
			cas.lock();
	 ...
	 * @return
	 */
	DoublyLinkedStackLong addRight(final DoublyLinkedStackLong r) {
		if (r != null) {
			cas.lock();
			try {
				size.incrementAndGet();//++;
				r.left = this;
				DoublyLinkedStackLong tr = right;
				this.right = r;
				r.right = tr;
				tr.left = r;
			} finally {
...
//		if(t!=null){
//			addLeft(new DoublyLinkedListInt(t, writeLock));
//			return true;
//		}else{
//			return false;
//		}
...
//	}
	 ...
	 * @return
	 */
	DoublyLinkedStackLong addLeft(final DoublyLinkedStackLong l) {
		if (l != null) {
			cas.lock();
			try {
				size.incrementAndGet();//+;
				l.right = this;
				DoublyLinkedStackLong tl = left;
				this.left = l;
				l.left = tl;
				tl.right = l;
			} finally {
	 ...
	 * @return
	 */
	DoublyLinkedStackLong searchObj(final long o){
		DoublyLinkedStackLong ref = this.right;
		while (ref != null) {
			if (ref == this) {
				break;
	...
	//@Override
	 long remove() {
		 cas.lock();
		try {
			DoublyLinkedStackLong tleft = left, tright = right;
			if (tleft != null)
				tleft.right = tright;
			if (tright != null)
				tright.left = tleft;
	...
	boolean contains(long o) {
		DoublyLinkedStackLong search = searchObj(o);
		if( search == null )
			return false;
		else
			return true;
...
//		if(e!=null){
//			addLeft(new DoublyLinkedStackInt(e, writeLock));
//			return true;
//		}else{
//			return false;
//		}
...
//	}
	...
	public long get(int index) {
		validateIndex(index);
		DoublyLinkedStackLong ref = getLinked(index);
		if(ref!=null)
			return ref.obj;
		else
			return Long.MIN_VALUE;
...
//	}
	...
	public int lastIndexOf(long o) {
		DoublyLinkedStackLong ref = this.left;
		int idx = size.get()-1;
		while (ref != null) {
			if (ref == this) {
				break;
